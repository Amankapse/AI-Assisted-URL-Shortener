...
## Stage 9D Amendment Single Render Web Service Packaging

- Approved deployment change: package Angular production output into the Spring Boot Docker image for one Render Web Service and one public URL.
- Source separation preserved: Angular remains under `frontend/`; backend remains under `src/`.
- Approved public behavior change: `GET /`, `/login`, `/register`, and `/app/**` serve Angular instead of root returning `401`.
...
## Stage 10 Premium Enterprise UX, Admin CMS, And Secure Admin Bootstrap

- Approved implementation scope: Stage 10 only.
- Approved migration: additive `V11__site_branding_and_content.sql`; V1-V10 were not modified.
- Approved backend scope: bounded `site_settings`, `content_pages`, `announcements`, `media_assets`, public read APIs under `/api/v1/site/**`, platform-admin CMS APIs under `/api/v1/admin/site/**`, audit action/resource-type additions, and `APP_BOOTSTRAP_ADMIN_EMAIL` startup promotion of an existing registered user.
- Approved frontend scope: real public landing page, safe public content pages, split login/register UX, left-rail app shell, `/app` overview, and lazy admin CMS routes `/app/admin/experience`, `/app/admin/content`, `/app/admin/announcements`, `/app/admin/media`.
- Explicitly not approved: new production dependencies, Lucide or other UI library, generated imagery, bank-logo imitation, default admin credentials, admin password env var, promotion endpoint, unrestricted CMS HTML/Markdown, `[innerHTML]`, `DomSanitizer.bypassSecurityTrustHtml(...)`, binary uploads, SVG media acceptance, redirect-path CMS lookups, broad `/api/v1/admin/**` permit, API-key CMS/admin authorization, Stage 11 work.
- Validation: `.\mvnw.cmd clean verify` passed with 125 backend tests; PostgreSQL Testcontainers started; Flyway V1-V11 validated/applied; Hibernate schema validation succeeded; JaCoCo line 85.07% / branch 60.15%; `.\mvnw.cmd dependency:tree` passed; `docker compose config` passed; `npm ci` passed; `npm test -- --watch=false` passed with 28 frontend tests; `npm run build` passed; `npm run build:render` passed with placeholder public env variables; `npm audit --audit-level=moderate` reported the known 3 moderate Angular CLI dev-chain findings; `docker build -t url-shortener-fullstack .` passed.
