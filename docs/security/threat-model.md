...
| Outbox poisoning or replay | Incorrect downstream delivery, repeated cache invalidation, or analytics overcount | Outbox payloads are bounded internal DTOs only; clients cannot submit outbox payloads; handlers are typed by controlled event enums; click events reuse stable IDs with idempotent persistence; admin API is read-only and omits raw payloads | Database administrators can still alter rows; external downstream integrations will need their own schema validation and replay controls |
